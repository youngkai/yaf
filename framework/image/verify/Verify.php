...
class Verify {
    ...
    public function check($code, $id = '') {
       ...
       // $secode = $_SESSION[$key];
        ...
        $secode = Yaf_Session::getInstance()->get($key);
        ...
        if(time() - $secode['verify_time'] > $this->config['expire']) {
            Yaf_Session::getInstance()->set($key, null);
            //$_SESSION[$key]=null;
            return false;
        }
        ...
        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            ...
            $this->config['reset'] && Yaf_Session::getInstance()->get($key);
            //$this->config['reset'] && $_SESSION[$key];
    ...
    public function entry($id = '') {
        ...
        $this->config['_image'] = imagecreate($this->config['imageW'], $this->config['imageH']);
        // 设置背景      
        imagecolorallocate($this->config['_image'], $this->config['bg'][0], $this->config['bg'][1], $this->config['bg'][2]);

        // 验证码字体随机颜色
        $this->config['_color'] = imagecolorallocate($this->config['_image'], mt_rand(1,150), mt_rand(1,150), mt_rand(1,150));
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '/ttfs/' . ($this->config['useZh'] ? 'zhttfs' : 'ttfs') . '/';
        ...
        if($this->config['useZh']){ // 中文验证码
            for ($i = 0; $i < $this->config['length']; $i++) {
                ...
                imagettftext($this->config['_image'], $this->config['fontSize'], mt_rand(-40, 40), $this->config['fontSize'] * ($i+1) * 1.5, $this->config['fontSize'] + mt_rand(10, 20), $this->config['_color'], $this->config['fontttf'], $code[$i]);
            }
        }else{
            for ($i = 0; $i < $this->config['length']; $i++) {
                ...
                imagettftext($this->config['_image'], $this->config['fontSize'], mt_rand(-40, 40), $codeNX, $this->config['fontSize'] * 1.6, $this->config['_color'], $this->config['fontttf'], $code[$i]);
            }
        }
        ...
        Yaf_Session::getInstance()->set($key.$id, $secode);
        ...
        // 输出图像
        imagepng($this->config['_image']);

        imagedestroy($this->config['_image']);
    }
    ...
    private function _writeCurve() {
        ...
        for ($px=$px1; $px<=$px2; $px = $px + 1) {
            if ($w!=0) {
                ...
                while ($i > 0) {	
                    imagesetpixel($this->config['_image'], $px + $i , $py + $i, $this->config['_color']);  // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px=$px1; $px<=$px2; $px=$px+ 1) {
            if ($w!=0) {
                ...
                $i = (int) ($this->config['fontSize'] / 5);
                while ($i > 0) {			
                    imagesetpixel($this->config['_image'], $px + $i, $py + $i, $this->config['_color']);
                    $i--;
                }
            }
        }
    ...
    private function _writeNoise() {
        ...
        for($i = 0; $i < 10; $i++){
            //杂点颜色
            $noiseColor = imagecolorallocate($this->config['_image'], mt_rand(150,225), mt_rand(150,225), mt_rand(150,225));
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->config['_image'], 5, mt_rand(-10, $this->config['imageW']),  mt_rand(-10, $this->config['imageH']), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
    ...
    private function _background() {
        ...
        @imagecopyresampled($this->config['_image'], $bgImage, 0, 0, 0, 0, $this->config['imageW'], $this->config['imageH'], $width, $height);

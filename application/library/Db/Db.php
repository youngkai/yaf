...
class db_Db {
    ...
    public static function getInstance($db_config='') {
		...
		if (is_object($config_str) && function_exists('spl_object_hash')) {
         		...
         		$guid= spl_object_hash((object)$config_str);
		...
		if(!isset(self::$_instance[$guid])){
            ...
            Yaf_Registry::set('db_Db', $obj);

			self::$_instance[$guid]	= $obj->factory($db_config);
		}

        Yaf_Registry::set('db_Db',self::$_instance[$guid]);
    ...
    public function factory($db_config='') {
        ...
        if(empty($db_config['dbms']))
            ...
            throw new Exception('there is no dbms');
        ...
        if(class_exists($class)) {
            ...
            throw new Exception('class undefine');
    ...
    private function parseConfig($db_config='') {
        ...
        if(!empty($db_config) && is_string($db_config)) {
            ...
            //如果配置为空，读取配置文件设置 如果设置了DB_DSN 则优先
            if(isset($config_db['dsn']) && 'pdo' != strtolower($config_db['type']) ) {
                ...
                $db_config = array (
                    ...
                    'hostport'  =>  $config_db['hostport'],
                    'database'  =>  $config_db['database'],
                    'dsn'       =>  isset($config_db['dsn']) ? $config_db['dsn'] : '',
                    'params'    =>  isset($config_db['params']) ? $config_db['params'] : array(),
                    'charset'   =>  isset($config_db['charset']) ? $config_db['charset'] : 'utf8',
                );
            }
        }
        return $db_config;
    ...
    protected function initConnect($master=true) {
        ...
        if(isset($config_db['deploy_type']) && 1 == $config_db['deploy_type'])

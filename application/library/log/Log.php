...
class Log{
     ...
     * @return void
     */
    public static function trance($message, $destination='', $level='ERR', $type=3, $extra='') {
        ...
        if(self::FILE == $type) {
        	if(!is_dir(APPLICATION_PATH . '/logs/')){
                        mkdir(APPLICATION_PATH.'/logs/', 0777, true);
            }
            if(!is_dir(APPLICATION_PATH .'/logs' . '/' . date('Ymd'))){
                        mkdir(APPLICATION_PATH . '/logs' . '/' . date('Ymd'), 0777, true);
            }
            if(empty($destination)){
                    $destination = APPLICATION_PATH . '/logs/' . date('Ymd') . '/' . date('y_m_d').'.log';
            }else{
            		$destination = APPLICATION_PATH . '/logs/' . date('Ymd') . '/' . $destination;
            }
            ...
            $extra = $extra ? $extra : 'extra message';
        }
